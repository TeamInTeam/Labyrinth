...
namespace game_map_visitor {
    ...
    struct GetWeapon : boost::static_visitor<std::pair<int32_t, int32_t>> {
        std::pair<int32_t, int32_t> operator() (Armoury &field) const {
        ...
        template<typename T>
        std::pair<int32_t, int32_t> operator() (T&) const {
            return std::make_pair(0, 0);
        }
    };
}
...
class GameMap {
 public:
    /*
      ...
      7 - Concrete.
     */
    GameMap(std::vector<std::vector<int32_t> > game_map_scheme);
    GameMap() = default;
    ...
    std::pair<int32_t, int32_t> GetWeapon(int32_t m, int32_t n);
 private:
