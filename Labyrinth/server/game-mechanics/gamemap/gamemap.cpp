...
GameMap::GameMap(std::vector<std::vector<int32_t> > game_map_scheme) {
    ...
    for (int32_t i = 0; i < size_m_; i++)
        for (int32_t j = 0; j < size_n_; j++) {
            ...
            case 4: {
                game_map_[i][j] = Armoury(int32_t(std::rand() % 5 + 1), int32_t(std::rand() % 5 + 1), int32_t(std::rand() % 5 + 1));
                break;
            }
            case 5: {
                game_map_[i][j] = Hospital(int32_t(std::rand() % 5 + 1));
                break;
            }
            case 6: {
                game_map_[i][j] = Bomb();
...
std::tuple<bool, int32_t, int32_t, int32_t> GameMap::GetItems(int32_t m, int32_t n) {
    ...
    boost::apply_visitor([](auto &aut) {game_map_visitor::PushItems()(aut, std::make_tuple(false, 0, 0, 0)); }, game_map_[m][n]);
    return result;
}
...
std::tuple<bool, bool, bool> GameMap::GetWeapon(int32_t m, int32_t n) {
