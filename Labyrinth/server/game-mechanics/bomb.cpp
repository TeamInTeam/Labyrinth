...
/*
std::tuple<std::pair<int32_t, ShowTurn>, std::vector<std::pair<int32_t, ShowOtherTurn>>, int32_t> Game::BombHandler(Turn turn) {
    ShowTurn show_turn;
    show_turn.turn = TurnTypes::BOMB;
    std::vector<std::pair<int32_t, ShowOtherTurn>> show_other_turns;

    if (!Check(show_turn, turn.m, turn.n, TurnTypes::BOMB)) {
    ...
    for (size_t i = 0; i < active_players_.size(); i++)
        if (static_cast<int32_t>(i) != index) {
            ...
            show_other_turn.turn = TurnTypes::BOMB;
    ...
    if (((type_of_field == FieldTypes::CONCRETE) || ((type_of_field == FieldTypes::WALL) && (game_map_.IsDestroyable(turn.m, turn.n)))) && !(IsHerePlayer(turn.m, turn.n))) {
        active_players_[index].bombs--;
        game_map_.ChangeType(turn.m, turn.n, FieldTypes::ROAD);
    } else if ((type_of_field == FieldTypes::ROAD) && !(IsHerePlayer(turn.m, turn.n)) && !((turn.m == 0) || (turn.n == 0) || (turn.m == sizes.first - 1) || (turn.n == sizes.second - 1))) {
        active_players_[index].bombs--;
        game_map_.ChangeType(turn.m, turn.n, FieldTypes::BOMB);
    }
